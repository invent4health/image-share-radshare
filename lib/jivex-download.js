...
import { waitForJivexZipReady } from './session-download.js';

export function isJivexPortalUrl(url) {
    return /jivexmobile/i.test(String(url || '').trim());
}
...
const JIVEX_DETECT_PASSWORD_FORMAT_SCRIPT = `
(function() {
    ${JIVEX_PASSWORD_HELPER}
    function normalizeFormatHint(text) {
        const t = String(text || '').trim();
        const patterns = [
            { re: /yyyy[-/.]mm[-/.]dd/i, format: 'YYYY-MM-DD', placeholder: 'YYYY-MM-DD' },
            { re: /dd[-/.]mm[-/.]yyyy/i, format: 'DD-MM-YYYY', placeholder: 'DD-MM-YYYY' },
            { re: /mm[-/.]dd[-/.]yyyy/i, format: 'MM-DD-YYYY', placeholder: 'MM-DD-YYYY' },
            { re: /dd\\.mm\\.yyyy/i, format: 'DD.MM.YYYY', placeholder: 'DD.MM.YYYY' },
        ];
        for (const p of patterns) {
            if (p.re.test(t)) return p;
        }
        if (/birth|geburtsdatum|date of birth|geb\\.?\\s*dat/i.test(t)) {
    ...
    const texts = [];
    for (const holder of document.querySelectorAll('div.widgetHolder')) {
        holder.querySelectorAll('.gwt-Label, label, span, .hint').forEach((el) => {
            const txt = (el.textContent || '').trim();
            if (txt && txt.length < 120) texts.push(txt);
        });
        const input = holder.querySelector('input.inputField, input');
        if (input) {
    ...
    document.querySelectorAll('.gwt-Label, label').forEach((el) => {
        const txt = (el.textContent || '').trim();
        if (txt.length < 120) texts.push(txt);
    });

    for (const text of texts) {
        const hint = normalizeFormatHint(text);
        if (hint) {
            return {
                ok: true,
                formatType: 'date',
                ...
                title: /passwort/i.test(text) ? 'Passwort' : 'Password',
                hint: hint.format,
            };
        }
    }
    ...
    const field = findJivexPasswordInput();
    if (field) {
        const holder = field.closest('.widgetHolder');
        const label = holder?.querySelector('.gwt-Label');
...
export async function detectJivexPasswordFormat(previewView, portalUrl) {
    if (!isJivexPortalUrl(portalUrl)) {
        return { ok: false, reason: 'Not a Jivex portal link' };
    }
    if (!previewView?.webContents || previewView.webContents.isDestroyed()) {
        return { ...DEFAULT_JIVEX_PASSWORD_FORMAT, reason: 'Browser preview is not available' };
    }

    const { restoreZoom } = await prepareJivexPortalInPreview(previewView, portalUrl);
    try {
        const result = await previewView.webContents.executeJavaScript(JIVEX_DETECT_PASSWORD_FORMAT_SCRIPT, true);
        restoreZoom?.();
        if (result?.ok && result.formatType === 'date' && result.format) {
            return result;
        }
        return {
            ...DEFAULT_JIVEX_PASSWORD_FORMAT,
            title: result?.title || DEFAULT_JIVEX_PASSWORD_FORMAT.title,
        };
    } catch (e) {
        restoreZoom?.();
        return {
...
function waitForJivexDownload(
    ...
    return new Promise((resolve, reject) => {
        ...
        const finish = async (filePath) => {
            ...
            try {
                await waitForJivexZipReady(filePath, {
                    stableMs: 1000,
                    timeoutMs: 20000,
                    minBytes: 50 * 1024,
                });
        ...
        const onDownload = (event, item) => {
            ...
            logJivexDownload('Portal download detected', { filename, url });

            if (!/^https?:/i.test(url) && !/\.zip$/i.test(filename)) return;
            ...
            try {
                ...
                logJivexDownload('setSavePath failed', { error: e?.message });
            }

            emit({ state: 'started', received: 0, total: 0, percent: 0 });

            item.on('updated', (_ev, state) => {
                if (settled || state !== 'progressing') return;
                ...
                emit({ state: 'progressing', received, total, percent });
            });

            item.once('done', (_ev, state) => {
                logJivexDownload('Native download finished', { state, path: targetPath });
                if (state === 'completed') {
                    finish(targetPath);
                } else {
                    fail(new Error(`Download ${state || 'failed'}`));
                }
            });
        };
        ...
        const pollTimer = setInterval(() => {
            if (settled || downloadTracked) return;
            let stat;
            try {
                stat = fs.statSync(targetPath);
            } catch {
                pollStableSince = Date.now();
                lastPolledSize = -1;
                return;
            }
            ...
            if (stat.size !== lastPolledSize) {
                lastPolledSize = stat.size;
                pollStableSince = Date.now();
                emit({
                    state: 'progressing',
                    received: stat.size,
            ...
            if (Date.now() - pollStableSince >= 2000) {
                logJivexDownload('Zip detected via filesystem poll', { targetPath, size: stat.size });
                downloadTracked = true;
                finish(targetPath);
            }
        }, 500);
