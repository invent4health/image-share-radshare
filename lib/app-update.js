...
export const UPDATE_PACKAGE_JSON_API_URL =
    'https://api.github.com/repos/invent4health/image-share-radshare/contents/package.json';
...
function httpsGetText(url, redirect = 0, extraHeaders = {}) {
            ...
            .get(
                ...
                (res) => {
                    if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
                        res.resume();
                        httpsGetText(res.headers.location, redirect + 1, extraHeaders).then(resolve, reject);
                        return;
                    }
...
export async function fetchRemotePackageVersion() {
    const apiUrl = withCacheBust(`${UPDATE_PACKAGE_JSON_API_URL}?ref=main`);
    try {
        const raw = await httpsGetText(apiUrl, 0, {
            Accept: 'application/vnd.github.raw+json',
        });
        const pkg = JSON.parse(raw);
        return String(pkg.version || '0.0.0');
    } catch {
        const raw = await httpsGetText(withCacheBust(UPDATE_PACKAGE_JSON_URL));
        const pkg = JSON.parse(raw);
        return String(pkg.version || '0.0.0');
    }
}
...
async function runNpmInstall(appRoot) {
    if (process.platform === 'win32') {
        await execFileAsync('cmd.exe', ['/d', '/s', '/c', 'npm install --no-fund --no-audit'], {
            cwd: appRoot,
            windowsHide: true,
...
async function downloadAndExtractUpdate(tempRoot, onStatus) {
    ...
    const zipBuffer = await httpsGetBuffer(withCacheBust(UPDATE_REPO_ZIP_URL));
...
async function launchUpdateScript({ tempRoot, sourceFolder, appRoot, preserveDir, logFile, elevated }) {
    ...
    if (elevated) {
        const bootstrapScript = path.join(tempRoot, 'elevate-update.ps1');
        fs.writeFileSync(
            bootstrapScript,
            [
        ...
        await execFileAsync(
            'powershell.exe',
            ['-NoProfile', '-ExecutionPolicy', 'Bypass', '-WindowStyle', 'Hidden', '-File', bootstrapScript],
            {
                windowsHide: true,
                maxBuffer: 1024 * 1024,
            },
    ...
    const detachScript = path.join(tempRoot, 'run-update.ps1');
    fs.writeFileSync(
        detachScript,
        [
            "$ErrorActionPreference = 'Stop'",
    ...
    await execFileAsync(
        'powershell.exe',
        ['-NoProfile', '-ExecutionPolicy', 'Bypass', '-WindowStyle', 'Hidden', '-File', detachScript],
        {
            windowsHide: true,
            maxBuffer: 1024 * 1024,
        },
...
export async function applyAppUpdate(appRoot, onStatus) {
    ...
    try {
        ...
        if (process.platform === 'win32') {
            notify(needsElevation ? 'Waiting for administrator approval…' : 'Restarting with update…');
            await launchUpdateScript({
                tempRoot,
                sourceFolder,
                appRoot,
                preserveDir,
