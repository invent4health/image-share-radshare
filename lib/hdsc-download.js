...
export async function waitForStableFile(filePath, stableMs = 2000) {
    let lastSize = -1;
...
export function snapshotDownloads(downloadsDir) {
    const map = new Map();
...
export function findNewestZipInDirs(dirs, beforeSnapshot, sinceMs) {
    let bestPath = null;
    let bestMtime = 0;
...
export async function enableSilentDownloadCdp(webContents, downloadDir, { pageBehavior = true } = {}) {
    ...
    try {
        ...
        if (pageBehavior) {
            commands.push(['Page.setDownloadBehavior', { behavior: 'allow', downloadPath: dir }]);
        }
...
export async function disableSilentDownloadCdp(webContents) {
    try {
...
export function scheduleSaveAsAutoConfirm(extraDelays = []) {
    const delays = [400, 900, 1500, 2500, 4000, 6000, 9000, ...extraDelays];
...
export function waitForHdscZipFile(session, watchDirs, beforeSnapshot, sinceMs, timeoutMs, options = {}) {
    const preventDefault = options.preventDefault !== false;
    const stableMs = options.stableMs ?? 2000;
    const log = options.log || logHdsc;
    const renameDownload = options.renameDownload;
    return new Promise((resolve, reject) => {
        ...
        const finish = (filePath) => {
            ...
            settled = true;
            cleanup();
            log('Zip file ready', { filePath });
            waitForStableFile(filePath, stableMs)
        ...
        const timer = setTimeout(
            () => fail(new Error('Timed out waiting for zip download')),
            timeoutMs
        );
        ...
        const onDownload = (event, item) => {
            ...
            if (typeof renameDownload === 'function') {
                safeName = renameDownload(safeName) || safeName;
            }
            ...
            item.on('updated', (_evt, state) => {
                if (state === 'interrupted' && !resumeAttempted && item.canResume?.()) {
                    resumeAttempted = true;
                    log('download interrupted — resuming', { filename });
            ...
            item.once('done', (_evt, state) => {
                ...
                log('will-download finished', { state, savePath });
                if (state === 'completed' && savePath) {
                    finish(savePath);
                    return;
                }
                if (state === 'cancelled' && !preventDefault) {
                    log('download cancelled — continuing folder watch', { savePath });
                }
                scanFolders();
            });
        };
